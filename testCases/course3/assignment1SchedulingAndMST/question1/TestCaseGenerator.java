package testCases.course3.assignment1SchedulingAndMST.question1;
...
public class TestCaseGenerator extends AbstractTestCaseGenerator {
  ...
  /**
   * Generate 4 unique example files for each of the specified number of jobs.
   *
   * <p>The job weights and lengths have the following bounds: [1, 100]
   *
   * @author beaunus
   * @param solverMainMethod the main method to be used to solve the generated input files
   * @param args the first index to be used in naming files, followed by a list of numbers of jobs
   */
  public static void random(Method solverMainMethod, String[] args) {
    ...
    // Iterate over each number of jobs and make 4 files with unique solutions.
    for (int i = 1; i < args.length; i++) {
      int numberOfJobs = Integer.parseInt(args[i]);

      System.out.println("Processing problem size = " + numberOfJobs);
